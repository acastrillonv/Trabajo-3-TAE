...
Con la realización de este proyecto, se desea crear una serie de modelos que permita predecir la accidentabilidad por tipo de accidente (Atropello, Choque, Caída de Ocupante, Volcamiento, Incendio, Otro) a nivel diario, semana y mensual.

Este proyecto ayudaría a entidades gubernamentales, la secretaría de movilidad, la alcadía de Medellín, ya que permitiría conocer un posible comportamiento de la cantidad de accidentes que se podrian tener en un intervalo de tiempo.
...
También con este trabajo, se creará un agrupamiento de los accidentes originados en los diferentes barrios de Medellín, y de acuerdo a una función propuesta, se podrá conocer una clasificación de accidentabilidad en cada barrio (Muy alta, alta, media, baja).
...
#Se crea un respaldo
data_corregida_aux <- data_corregida
```
...
Ya con el dataset anteriormente construido, se van a crear una serie de modelos por cada tipo de accidente, esto nos permitirá conocer el nivel de accidentabilidad dependiendo del tipo de accidente.
...
```{r}
#Modelo final
modelo_atropello <- knnreg(x=X_tr_acp, y = Y_tr$Numero_Accidente, k = 25)
```
...
```{r}
#Modelo final
modelo_choque <- knnreg(x=X_tr_acp, y = Y_tr$Numero_Accidente, k = 29)
```
...
```{r}
#Modelo final
modelo_ca <- knnreg(x=X_tr_acp, y = Y_tr$Numero_Accidente, k = 30)
```
...
```{r}
#Modelo final
modelo_volcamiento <- knnreg(x=X_tr_acp, y = Y_tr$Numero_Accidente, k = 30)
```
...
```{r}
#Modelo final
#modelo_incendio <- knnreg(x=X_tr_acp, y = Y_tr$Numero_Accidente, k = 2)
```
...
```{r}
#Modelo final
modelo_otro <- knnreg(x=X_tr_acp, y = Y_tr$Numero_Accidente, k = 30)
```

## 3.2 - Comportamiento de la accidentabilidad en los años 2021 y 2022

```{r}
#fecha festivos
festivos_2021 <- c("2021-01-01","2021-01-11","2021-03-22","2021-04-01","2021-04-02","2021-04-04","2021-05-01","2021-05-17","2021-06-07","2021-07-05","2021-07-20","2021-08-07","2021-08-15","2021-10-18","2021-11-08","2021-11-15","2021-12-08","2021-12-25")

festivos_2022 <- c("2022-01-01","2022-01-10","2022-03-21","2022-04-14","2022-04-15","2022-04-17","2022-05-01","2022-05-30","2022-06-20","2022-06-27","2022-07-04","2022-07-20","2022-08-07","2022-08-17","2022-10-17","2022-11-07","2022-11-14","2022-12-08","2022-12-25")

fecha_festivos2 <- as.Date(c(festivos_2021,festivos_2022))


# df 2021
df_2021 <- as.data.frame(seq(from=as.Date("2021-01-01"), to=as.Date("2021-12-31"), by=1))
names(df_2021) <- c("Fecha")
df_2021$Dia <- day(df_2021$Fecha)
df_2021$Semana <- week(df_2021$Fecha)
df_2021$Quincena <- sapply(df_2021$Dia, iden_quincena)
df_2021 <- mutate(df_2021,Festivo=ifelse((Fecha %in% fecha_festivos2), "Si", "No"))
df_2021$DiaDeLaSemana <- weekdays(df_2021$Fecha)
df_2021$Finde <- sapply(df_2021$DiaDeLaSemana, iden_weekend)
df_2021$MES <- month(df_2021$Fecha)
...
df_2021$Dia <- as.numeric(df_2021$Dia)
df_2021$Semana <- as.numeric(df_2021$Semana)
df_2021$DiaDeLaSemana <- as.factor(df_2021$DiaDeLaSemana)
df_2021$Festivo <- as.factor(df_2021$Festivo)
df_2021$Finde <- as.factor(df_2021$Finde)
df_2021$MES <- as.factor(df_2021$MES)
df_2021$Quincena <- as.factor(df_2021$Quincena)

# df 2022
df_2022 <- as.data.frame(seq(from=as.Date("2022-01-01"), to=as.Date("2022-12-31"), by=1))
names(df_2022) <- c("Fecha")
df_2022$Dia <- day(df_2022$Fecha)
df_2022$Semana <- week(df_2022$Fecha)
df_2022$Quincena <- sapply(df_2022$Dia, iden_quincena)
df_2022 <- mutate(df_2022,Festivo=ifelse((Fecha %in% fecha_festivos2), "Si", "No"))
df_2022$DiaDeLaSemana <- weekdays(df_2022$Fecha)
df_2022$Finde <- sapply(df_2022$DiaDeLaSemana, iden_weekend)
df_2022$MES <- month(df_2022$Fecha)
...
df_2022$Dia <- as.numeric(df_2022$Dia)
df_2022$Semana <- as.numeric(df_2022$Semana)
df_2022$DiaDeLaSemana <- as.factor(df_2022$DiaDeLaSemana)
df_2022$Festivo <- as.factor(df_2022$Festivo)
df_2022$Finde <- as.factor(df_2022$Finde)
df_2022$MES <- as.factor(df_2022$MES)
df_2022$Quincena <- as.factor(df_2022$Quincena)

#Procesamiento año 2021

##Estandarizacion variables categorica
variables_cat <- sapply(df_2021, is.factor)
data_cat <- df_2021[variables_cat]
data_cat <- subset(data_cat,select = -c(DiaDeLaSemana))
onehotencoding <- dummyVars(~.,data = data_cat)
data_cat_dummy <- as.data.frame(predict(onehotencoding,data_cat))

df_2021 <- subset(df_2021, select = -c(DiaDeLaSemana,MES))
df_2021 <- cbind(df_2021,data_cat_dummy)
Fecha_2021 <- df_2021$Fecha
df_2021 <- subset(df_2021 , select = -c(Quincena,Festivo,Finde,Fecha))
##Se centra la información usando la media y la desviacion de los datos con los que fue entrenado el modelo.
df_2021_std <- scale(df_2021,center = media_X_tr, scale = sd_X_tr)
df_2021_std <- as.data.frame(df_2021_std)

#Procesamiento año 2022

##Estandarizacion variables categorica
variables_cat <- sapply(df_2022, is.factor)
data_cat <- df_2022[variables_cat]
data_cat <- subset(data_cat,select = -c(DiaDeLaSemana))
onehotencoding <- dummyVars(~.,data = data_cat)
data_cat_dummy <- as.data.frame(predict(onehotencoding,data_cat))

df_2022 <- subset(df_2022, select = -c(DiaDeLaSemana,MES))
df_2022 <- cbind(df_2022,data_cat_dummy)
Fecha_2022 <- df_2022$Fecha
df_2022 <- subset(df_2022 , select = -c(Quincena,Festivo,Finde,Fecha))
##Se centra la información usando la media y la desviacion de los datos con los que fue entrenado el modelo.
df_2022_std <- scale(df_2022,center = media_X_tr, scale = sd_X_tr)
df_2022_std <- as.data.frame(df_2022_std)
```

### 3.2.1- Predicción de la accidentabilidad para tipo de accidente "Atropello"

```{r}
#Se calculan las predicciones
ACP <- predict(ACP_c1, newdata= df_2021_std)
df_2021_acp <- data.frame(ACP[,1:6])
predicciones_2021 <- predict(modelo_atropello, newdata = df_2021_acp)
predicciones_2021 <- as.data.frame(predicciones_2021)
predicciones_2021$Fecha <- Fecha_2021

ACP <- predict(ACP_c1, newdata= df_2022_std)
df_2022_acp <- data.frame(ACP[,1:6])
predicciones_2022 <- predict(modelo_atropello, newdata = df_2022_acp)
predicciones_2022 <- as.data.frame(predicciones_2022)
predicciones_2022$Fecha <- Fecha_2022
```

```{r}
plot(predicciones_2021$Fecha,predicciones_2021$predicciones_2021, type = "l", main = "Predicción de accidentes tipo Atropello para el año 2021", xlab = "Año 2021", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 1.** Predicción de accidentes tipo Atropello para el año 2021</center>

<br>

```{r}
plot(predicciones_2022$Fecha,predicciones_2022$predicciones_2022, type = "l", main = "Predicción de accidentes tipo Atropello para el año 2022", xlab = "Año 2022", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 2.** Predicción de accidentes tipo Atropello para el año 2022</center>
...
### 3.2.2- Predicción de la accidentabilidad para tipo de accidente "Choque"

```{r}
#Se calculan las predicciones
ACP <- predict(ACP_c2, newdata= df_2021_std)
df_2021_acp <- data.frame(ACP[,1:2])
predicciones_2021 <- predict(modelo_choque, newdata = df_2021_acp)
predicciones_2021 <- as.data.frame(predicciones_2021)
predicciones_2021$Fecha <- Fecha_2021

ACP <- predict(ACP_c2, newdata= df_2022_std)
df_2022_acp <- data.frame(ACP[,1:2])
predicciones_2022 <- predict(modelo_choque, newdata = df_2022_acp)
predicciones_2022 <- as.data.frame(predicciones_2022)
predicciones_2022$Fecha <- Fecha_2022
```

```{r}
plot(predicciones_2021$Fecha,predicciones_2021$predicciones_2021, type = "l", main = "Predicción de accidentes tipo Choque  para el año 2021", xlab = "Año 2021", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 3.** Predicción de accidentes tipo Choque para el año 2021</center>

<br>

```{r}
plot(predicciones_2022$Fecha,predicciones_2022$predicciones_2022, type = "l", main = "Predicción de accidentes tipo Choque  para el año 2022", xlab = "Año 2022", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 4.** Predicción de accidentes tipo Choque para el año 2022</center>

<br>

Entre las Gráficas 3 y 4 podemos evidenciar una mayor cantidad de picos bajos en los meses de mitad del año, de resto, las gráficas tienen aproximadamente el mismo comportamiento en ambos años tanto en los picos bajos como en los picos altos. Esto significa que el comportamiento de las accidentes de tipo Choque va a ser muy similar en ambos años.

### 3.2.3- Predicción de la accidentabilidad para tipo de accidente "Caída de Ocupante"

```{r}
#Se calculan las predicciones
ACP <- predict(ACP_c3, newdata= df_2021_std)
df_2021_acp <- data.frame(ACP[,1:2])
predicciones_2021 <- predict(modelo_ca, newdata = df_2021_acp)
predicciones_2021 <- as.data.frame(predicciones_2021)
predicciones_2021$Fecha <- Fecha_2021

ACP <- predict(ACP_c3, newdata= df_2022_std)
df_2022_acp <- data.frame(ACP[,1:2])
predicciones_2022 <- predict(modelo_ca, newdata = df_2022_acp)
predicciones_2022 <- as.data.frame(predicciones_2022)
predicciones_2022$Fecha <- Fecha_2022
```

```{r}
plot(predicciones_2021$Fecha,predicciones_2021$predicciones_2021, type = "l", main = "Predicción de accidentes tipo Caída de Ocupante para el año 2021", xlab = "Año 2021", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 5.** Predicción de accidentes tipo Caída de Ocupante para el año 2021</center>

<br>

```{r}
plot(predicciones_2022$Fecha,predicciones_2022$predicciones_2022, type = "l", main = "Predicción de accidentes tipo Caída de Ocupante  para el año 2022", xlab = "Año 2022", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 6.** Predicción de accidentes tipo Caída de Ocupante para el año 2022</center>

<br>

Luego de observar las Gráficas 5 y 6 podemos observar que, en la Gráfica 6, hay picos más profundos que en la Gráfica 5. Además, en los finales del mes de Junio hay una mayor cantidad de picos bajos en la Gráfica 6. También podemos ver que los picos bajos en la gráfica 6 son más extremos que los picos bajos que a los que llegan en la Gráfica 5.

### 3.2.4- Predicción de la accidentabilidad para tipo de accidente "Volcamiento"

```{r}
#Se calculan las predicciones
ACP <- predict(ACP_c4, newdata= df_2021_std)
df_2021_acp <- data.frame(ACP[,1:3])
predicciones_2021 <- predict(modelo_volcamiento, newdata = df_2021_acp)
predicciones_2021 <- as.data.frame(predicciones_2021)
predicciones_2021$Fecha <- Fecha_2021

ACP <- predict(ACP_c4, newdata= df_2022_std)
df_2022_acp <- data.frame(ACP[,1:3])
predicciones_2022 <- predict(modelo_volcamiento, newdata = df_2022_acp)
predicciones_2022 <- as.data.frame(predicciones_2022)
predicciones_2022$Fecha <- Fecha_2022
```

```{r}
plot(predicciones_2021$Fecha,predicciones_2021$predicciones_2021, type = "l", main = "Predicción de accidentes tipo Volcamiento para el año 2021", xlab = "Año 2021", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 7.** Predicción de accidentes tipo Volcamiento para el año 2021</center>

<br>

```{r}
plot(predicciones_2022$Fecha,predicciones_2022$predicciones_2022, type = "l", main = "Predicción de accidentes tipo Volcamiento  para el año 2022", xlab = "Año 2022", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 8.** Predicción de accidentes tipo Volcamiento para el año 2022</center>

<br>

La variación más notoria es en los primeros 3 meses en donde hay mayor cantidad de picos altos en la Gráfica 8. Por otro lado, vemos en la Gráfica 7, en el mes de agosto, el pico mas alto de los 2 años de predicción. En el mes de noviembre se ve un comportamiento un poco diferente, siendo mas variable en la gráfica del año 2022 (Gráfica 8).

### 3.2.5- Predicción de la accidentabilidad para tipo de accidente "Otro"

```{r}
#Se calculan las predicciones
ACP <- predict(ACP_c6, newdata= df_2021_std)
df_2021_acp <- data.frame(ACP[,1:2])
predicciones_2021 <- predict(modelo_otro, newdata = df_2021_acp)
predicciones_2021 <- as.data.frame(predicciones_2021)
predicciones_2021$Fecha <- Fecha_2021

ACP <- predict(ACP_c6, newdata= df_2022_std)
df_2022_acp <- data.frame(ACP[,1:2])
predicciones_2022 <- predict(modelo_otro, newdata = df_2022_acp)
predicciones_2022 <- as.data.frame(predicciones_2022)
predicciones_2022$Fecha <- Fecha_2022
```

```{r}
plot(predicciones_2021$Fecha,predicciones_2021$predicciones_2021, type = "l", main = "Predicción de accidentes tipo Otro para el año 2021", xlab = "Año 2021", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 9.** Predicción de accidentes tipo Otro para el año 2021</center>

<br>

```{r}
plot(predicciones_2022$Fecha,predicciones_2022$predicciones_2022, type = "l", main = "Predicción de accidentes tipo Otro  para el año 2022", xlab = "Año 2022", ylab = "Número de accidentes", col="red")
```

<center>**Gráfica 10.** Predicción de accidentes tipo Otro para el año 2022</center>

<br>

En las Gráficas 9 y 10 podemos encontrar un comportamiento muy similar a lo largo del año, lo que significa que el modelo predijo muy pocas variaciones entre el año 2021 y 2022 en relación a las accidentes de tipo diferente a choque, volcamiento, caída de ocupante o atropello, a excepción del mes septiembre donde podemos ver un pico más alto en la Gráfica 10.

# 4- Agrupamiento de los barrios de Medellín de acuerdo a su accidentabilidad

Para poder crear la clasificación de la accidentabilidad en los barrios de la ciudad de Medellín, se crea una base de datos en la que se agrupan todos los registros de accidentes por los diferentes barrios y las comunas, esto se puede ver en la Tabla 35.
...
calculo_indice <- data_corregida %>% group_by(BARRIO,COMUNA) %>%
          summarise(cantidad_accidentes = n(),
                    ...
                    Porcentaje = Choque/cantidad_accidentes)
calculo_indice <- calculo_indice[calculo_indice$BARRIO!= '""',]
calculo_indice <- calculo_indice[calculo_indice$COMUNA!= '""',]

clasif <- function(p){
  if (p<0.34){
    pn <- "Leve"
  }
  else if (p>=0.34 & p<0.67){
    pn <- "Media"
  }
  else{
    pn <- "Alta"
...
calculo_indice$Riesgo <- sapply(calculo_indice$Porcentaje, clasif)

calculo_indice <- subset(calculo_indice, select = -c(cantidad_accidentes,Choque,Porcentaje))

kable(head(calculo_indice),caption = "Agrupación de los accidentes por barrios y comunas") %>% 
  kable_styling(full_width = F,position = "center") %>% 
  kable_minimal()
```

<center>**Tabla 35.** Agrupación de los accidentes por barrios y comunas de la ciudad de Medellín</center>

<br>

En la Tabla 35, podemos observar que se tiene también el conteo de los accidentes tipo "Choque" producidos en cada barrio de las diferentes comunas . Esto se hizo ya que también se crea una columna llamada Porcentaje en la cual se hizo calculó el porcentaje que representan los accidentes tipo Choque con respecto a la cantidad total de accidentes por barrios.

Con los porcentajes, se realizó una clasificación del riesgo de choque en tres niveles (Alta, Media,Leve) de la siguiente forma:

-   Porcentajes menores a 34%, son clasificados riesgo Leve.

-   Porcentajes entre 34% y 67%, son clasificados riesgo Medio.

-   Porcentajes mayores a 67%, son clasificados riesgo Alto.

Para la construcción del mapa, se utilizaron unos archivos encontrados en medata.gov.co, en los cuales se encuentra la información de las divisiones de los barrios y comunas de la ciudad de Medellín en forma de polígonos. Con esta información, se crea un mapa utilizando la librería "leaflet".
...
barrios=readOGR("myapp/www/.",layer = "Limite_Barrio_Vereda_Catastral")
```

```{r}

mapa=leaflet(barrios)
mapa=addTiles(mapa)



color <- colorFactor(palette = c("#d90000",  "#ffa333", "#006400"), 
                            levels = c("Alta", "Media","Leve"))

mapa = leaflet() %>% addPolygons(data=barrios,
                  weight = 1,
                  color = "white",
                  fillColor =color(calculo_indice$Riesgo),
                  fillOpacity = 0.8,
                  
                  label=paste0("<p style='font-size:20px'> <strong>Barrio: </strong>",barrios$NOMBRE_BAR,
                               "<br><strong>Riesgo: </strong>",calculo_indice$Riesgo,
                               "<br><strong>Comuna: </strong>",barrios$NOMBRE_COM
                  )%>% lapply(htmltools::HTML),
      )%>%
      addTiles() %>%
  addLegend("bottomright",
    colors = c("#d90000",  "#ffa333", "#006400"),
    labels = c("Alta", "Media","Leve"),
    title = "Niveles",
    opacity = 1)
mapa
```

<center>**Mapa 1.** Nivel de riesgo de accidente tipo "Choque" en los barrios y comunas de la ciudad de Medellín</center>

<br>

En el Mapa 1, podemos indentificar de manera visual que barrios de la ciudad de Medellín poseen una clasificación Alta, Media o Leve de acuerdo al porcentaje de choques ocurrido cada uno de ellos.

Con el Mapa 1, usuarios como la secretaría de movilidad o la alcadía de Medellín, podrian implementar estrategias que permitan reducir los niveles de accidentes tipo choque en los barrios identificados con riesgo alto.

Algunas de las medidas que se podrían implementar son

-   Mejorar las vías, esto ayudaría a disminuir choques entre vehículos por problemas en las vías.

-   Monitoreo de la velocidad de conducción.

-   Aumentar las señales de tránsitos en las vías o semáforos, que permitan estructurá un correcto flujo vial.

# 5- Links

-   Para conocer la aplicación web que creamos, haz click, <a href='https://sebasrendon12.shinyapps.io/my3app/' target="_blank"> aquí </a> .

-   Para conocer el video donde explicamos acerca del funcionamiento de la aplicación, haz click, <a href='https://youtu.be/oE-yhAHgkrA' target="_blank"> aquí </a> .
