...
X_tr <- subset(X_tr, select = c(Festivo.Si, Festivo.No, Semana))
...
X_vl <- subset(X_vl, select = c(Festivo.Si, Festivo.No, Semana))
Y_vl <- subset(Y_val, Y_val$CLASE_ACCIDENTE=="Atropello")
Y_vl <- subset(Y_vl, select = -c(CLASE_ACCIDENTE))

k <- 2:80

resultadosC1 <- sapply(k, optimo_K, X_t=X_tr,Y_t=Y_tr,X_v=X_vl,Y_v=Y_vl)
...
```{r}
X_tr_acp <- data.frame(ACP$x[,1:2])
ACP_vl <- predict(ACP, newdata= X_vl)
X_vl_acp <- data.frame(ACP_vl[,1:2])
```

En la Tabla 23, podemos observar que el mejor K
...
<center>**Tabla 23.** Análisis del valor K para la variable "Atropello"</center>

<br>

```{r}
resultadosC1$Mae_t <- as.numeric(resultadosC1$Mae_t)
resultadosC1$Mae_v <- as.numeric(resultadosC1$Mae_v)
resultadosC1$Variacion <- as.numeric(resultadosC1$Variacion)

summary(resultadosC1$Variacion)
```

<center>**Tabla 24.** Análisis del variación del error MAE para la variable "Atropello"</center>
...
resultadosC2 <- sapply(k, optimo_K, X_t=X_tr,Y_t=Y_tr,X_v=X_vl,Y_v=Y_vl)
...
resultadosC3 <- sapply(k, optimo_K, X_t=X_tr,Y_t=Y_tr,X_v=X_vl,Y_v=Y_vl)
...
resultadosC4 <- sapply(k, optimo_K, X_t=X_tr,Y_t=Y_tr,X_v=X_vl,Y_v=Y_vl)
...
resultadosC5 <- sapply(k, optimo_K, X_t=X_tr,Y_t=Y_tr,X_v=X_vl,Y_v=Y_vl)
...
resultadosC6 <- sapply(k, optimo_K, X_t=X_tr,Y_t=Y_tr,X_v=X_vl,Y_v=Y_vl)
...
```{r}
filter_evaluator <- filterEvaluator('determinationCoefficient')
p <- selectKBest(data=data_modelo, class='Numero_Accidente', filter_evaluator)
```
