...
data_corregida <- mutate(data_corregida, Festivo=ifelse((FECHA_ACCIDENTE %in% fecha_festivas), 1, 0) )
...
data_corregida$Quincena <- as.numeric(data_corregida$Quincena)
data_corregida$Festivo <- as.numeric(data_corregida$Festivo)
data_corregida$Finde <- as.numeric(data_corregida$Finde)
...
```{r}
#Ejemplo de como agrupar
#accFechas <- data_corregida %>% 
...
Como primer paso, se va a crear un nuevo dataset, en el que podemos observar la cantidad de cada tipo de accidente (Choque, Atropello, Caída de Ocupante, Volcamiento, Incendio, Otro) por fecha registrada. El dataset nuevo se puede observar en la Tabla 22.
...
data_corregida$Quincena <- sapply(data_corregida$Dia, iden_quincena) 
data_modelo <- mutate(data_modelo, Festivo=ifelse((FECHA_ACCIDENTE %in% fecha_festivas), 1, 0) )
...
data_modelo$Festivo <- as.numeric(data_modelo$Festivo)
data_modelo$Finde <- as.numeric(data_modelo$Finde)
...
X_tr_std <- scale(data_num,center = media_X_tr, scale = sd_X_tr)
X_tr_std <- as.data.frame(X_tr_std)
X_tr_std$CLASE_ACCIDENTE <- X_train$CLASE_ACCIDENTE
```

Debido a que se va a trabajar con la técnica de KNN, se tiene que encontrar el óptimo K para cada modelo.

```{r}
#función para el calculo del mae
MAE <- function(X,Y){
  
}

#función para encontrar el valor optimo K
optimo_K <- function(X,Y){
